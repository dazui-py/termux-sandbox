...
int cmd_create(int argc, char *argv[]) {
    ...
    if (!rootfs_source) {
        fprintf(stderr, "No rootfs artifact found in ~/.termux-sandbox/cache/rootfs/.\n");
        fprintf(stderr, "Build or copy a .tar.zst/.tar.gz rootfs artifact there first.\n");
        return 1;
    }
    ...
    if (!logs_path || mkdir_p(logs_path, 0755) != 0) {
        perror(logs_path ? logs_path : "logs path");
        free(logs_path);
        free(sandbox_path);
        free(rootfs_path);
        free(rootfs_source);
...
int cmd_list(int argc, char *argv[]) {
    ...
    if (!boxes_dir) {
        fprintf(stderr, "Failed to build boxes path\n");
        return 1;
    }
