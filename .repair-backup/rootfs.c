...
char *rootfs_get_cached(void) {
    ...
    int n = snprintf(search_path, sizeof(search_path), "%s/rootfs", cache_dir);
    if (n < 0 || (size_t)n >= sizeof(search_path)) return NULL;

    if (mkdir_p(search_path, 0755) != 0) return NULL;
